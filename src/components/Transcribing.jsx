...
export const Transcribing = (props) => {
  ...
  /*return (
    ...
    return (
        <div className='flex items-center flex-1 flex-col justify-center gap-10 md:gap-14 text-center pb-24 p-4 bg-black text-white'>
          <div className='flex flex-col gap-2 sm:gap-4'>
          ...
          <div className='flex flex-col gap-2 sm:gap-3 max-w-[500px] mx-auto w-full'>
            {[0, 1, 2].map((val) => {
              return (
                <div 
                  key={val} 
                  className={`rounded-full h-2 sm:h-3 bg-gray-500 loading loading${val}`}
                ></div>
              );
            })}
          </div>
